...
test('게스트 입퇴사는 명찰 지급과 퇴사를 기록한다', async ({ page }) => {
  ...
  await expect(page.getByText('입사 프로세스: 유니폼 지급 완료')).toBeVisible()
  await page.getByRole('checkbox', { name: '노트북 지급' }).click()
  await expect(page.getByText('입사 프로세스: 노트북 지급 완료')).toBeVisible()
  await expect(page.getByRole('heading', { name: '퇴사 프로세스' })).toBeVisible()
  await page.getByRole('checkbox', { name: '명찰 회수' }).click()
  await expect(page.getByText('퇴사 프로세스: 명찰 회수 완료')).toBeVisible()
  await page.getByRole('checkbox', { name: '유니폼 회수' }).click()
