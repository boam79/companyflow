...
| 역할 | 기본 업무 |
|---|---|
| 운영 관리자 | 회사 생성, 최초 회사 관리자 연결, 허용 모듈 |
...
운영 관리자와 회사 관리자의 권한은 구분한다. 운영 관리자는 멤버십을 자동으로 갖지 않는다. 다른 회사의 사람·재고 원본은 그 회사 지정 PC에서만 연다. 운영 계정은 허용 모듈만 다룬다. 최초 운영 관리자 권한은 서버의 별도 운영 절차로 부여하고, 일반 사용자가 가입·프로필 수정으로 자신에게 부여할 수 없게 한다. 상세는 D-32·D-33, `DOC/tenants.md`.
