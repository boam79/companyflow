...
- 한 회사는 최초 관리자 한 명이다. 회사 설정에 추가 사람 초대를 두지 않는다. 새 회사 연결은 회사 생성의 최초 관리자 이메일만 쓴다. 팔 회사 sqlite에는 본사 샘플(김담당·본사창고)을 넣지 않는다. 회사 관리자 설정에 운영 모듈 안내를 두지 않는다. 직원이 없으면 입퇴사 명찰 칸을 두지 않는다. 사용 가능이면 초기 설정 메뉴를 숨긴다. 업무 화면에 VFS 이름과 DB 다시 열기를 두지 않는다.
- 파일을 고치기 전에 읽는다.
