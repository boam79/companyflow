...
- T7 완료는 지정 Chrome 본사(HQ01) 원본에서 AC-04·13·23을 확인한 뒤에만 선언한다. T8은 같은 원본 `/contracts` OCR·초안·원본, T9는 `/data` 수신 키·암호화 묶음과 HTTPS E2E 게스트 백업 잔재 0을 확인한 뒤에만 선언한다. AC-14 통계 메뉴와 `/reports`·PUR-02 결재·P3는 사용자가 켜기 전에는 되돌리지 않는다.
- 운영 JWT만으로는 다른 회사 QR 수신함·원본 장치·수신 키를 열지 않는다. 빈 QR 만들기와 수신함 반영은 그 회사 관리자만. 계약 PDF는 스크립트·eval을 끈다. CSP `connect-src`는 전용 프로젝트 호스트만, `script-src-attr 'none'`.
