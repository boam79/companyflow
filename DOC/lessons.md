...
- 독립 회사·운영 계정은 `DOC/tenants.md`를 따른다. 본사를 연 채 다른 회사 사람·재고를 보여 주지 않는다. 운영 계정 업무 목록에 지점을 넣지 않는다. 회사 관리에서 전체 목록을 업무 세션에 넣지 않는다. `create_company`가 운영자 본인을 고객사 관리자로 붙이지 않는다.
- 파일을 고치기 전에 읽는다.
