...
마지막 갱신: 2026-09-20  
현재 역할: Executor  
구현: T1~T6 완료. T8 계약 OCR 사용자 확인. T7 이관·QR 상세 확인. AST-02 수령 자산화 진행.  
원격: https://github.com/boam79/companyflow.git

## 지금 하는 일

책상·컴퓨터 발주를 수령하면 재고가 아니라 개별 자산으로 등록한다. 복사용지 수불부는 그대로다. 직원 배정은 하지 않는다(D-30).
...
- 2026-09-20: AST-02 구매 수령 직접 자산화(가구·컴퓨터). 복사용지는 창고만.
...
- AST-01 구매 원본은 수령 자산화 때 발주 번호를 자산에 남김. **이번 배포에서 확인 예정.**
- AST-02 구매 수령에서 창고/자산 배분. **이번 배포: 가구는 전량 자산, 복사용지는 창고.**
