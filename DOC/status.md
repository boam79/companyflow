...
마지막 갱신: 2026-09-24  
현재 역할: Executor  
회사는 서로 독립이다. 운영 관리자는 등록만 하고 다른 회사 원본을 열지 않는다. 백업·복원은 열지 않음. T7 완료 아님.  
원격: https://github.com/boam79/companyflow.git

## 지금 하는 일

운영 계정 업무 목록은 멤버로 연결된 회사만. 본사 설정에서 다른 회사 모듈·사람을 열지 않음. 지점 멤버십은 운영 계정에서 뗐다.
...
- AC-02·27 회사는 서로 독립. 운영 권한만으로는 다른 회사 원본을 열지 않음. HQ01이 다른 회사 모듈을 바꾸던 경로는 제거.
- 확인 완료. 지점 구매·재고 끔 → 안내와 회사 선택 → 본사로 바꾸면 목록. 저장된 내용 유지. **(outdated: 본사·지점 부모 관계 아님)**
