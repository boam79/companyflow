...
마지막 갱신: 2026-09-22  
현재 역할: Executor  
데이터 관리. 회사 관리자가 예약된 원본 PC를 확정. 백업·복원은 열지 않음. T7 완료 아님.  
원격: https://github.com/boam79/companyflow.git

## 지금 하는 일

`/data`에서 회사 관리자가 이 PC를 원본으로 확정한다. 사용자 계정에는 버튼이 없다. 백업·복원 버튼은 없다.
...
- 데이터 관리 확인. 이 기기 예, 전송 없음, 관리자 PC 예약만 됨, 처리 확인 없음, 최근 백업 없음.
- 원본 PC 확정은 회사 관리자만. 백업·복원 화면은 없음. 복원은 T9
