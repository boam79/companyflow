...
마지막 갱신: 2026-09-22  
현재 역할: Executor  
회사 설정. 회사 카드마다 그 파일의 표시를 보여 줌. 백업·복원은 열지 않음. T7 완료 아님.  
원격: https://github.com/boam79/companyflow.git

## 지금 하는 일

`/settings`에서 연 회사가 아니어도 각 카드는 그 회사 파일의 통화·자리·시간대를 보여 준다. 백업·복원은 열지 않는다.
...
- 표시 분리 확인. 본사는 세계시·달러·12345. 지점은 서울·원·12,345. 카드가 닫혀 있어도 그 파일 값을 보여 줌. 백업·복원 화면은 없음. 복원은 T9
