...
지정 Chrome 품목의 같은 이름 두 줄을 코드 있는 한 줄로 모은다. 결재·통계는 넣지 않는다.
...
- 2026-09-20: 같은 이름 품목 정리. `query`를 뽑아 쓰면 `this.send`가 깨져 화면에 오류가 난다. 화살표로 감싼다. 결재·통계 없음.
...
- MST-01·02·03·05 공급사·거래처 연락처 없음. MST-01 코드·단위, MST-02 구매 구분, 같은 이름 비활성화, MST-04 최소재고는 이번 배포. 지정 Chrome 확인 전.
