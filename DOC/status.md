...
마지막 갱신: 2026-09-22  
현재 역할: Executor  
회사 설정. 자리 구분은 이 회사 원본에만 저장. 백업·복원은 열지 않음. T7 완료 아님.  
원격: https://github.com/boam79/companyflow.git

## 지금 하는 일

`/settings`에서 회사 관리자가 숫자 자리 구분을 이 PC의 회사 원본에 저장한다. 기본은 사용. 백업·복원은 열지 않는다.
...
- 표시 통화 확인. 본사 원본은 달러. 자리 구분은 같은 원본 meta. 백업·복원 화면은 없음. 복원은 T9
